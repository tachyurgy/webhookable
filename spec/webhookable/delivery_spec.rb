...
RSpec.describe Webhookable::Delivery do
  ...
  describe '.deliver' do
    ...
    it 'increments attempt count' do
      expect do
        described_class.deliver(delivery)
      end.to change { delivery.reload.attempt_count }.by(1)
    end
  ...
  describe 'inbox mode' do
    ...
    it 'stores webhook in inbox instead of delivering' do
      expect do
        described_class.deliver(delivery)
      end.to change { Webhookable::InboxEntry.count }.by(1)
  ...
  describe 'instrumentation' do
    ...
    it 'instruments webhook delivery' do
      ...
      ActiveSupport::Notifications.subscribe('webhook.delivered') do |_name, _start, _finish, _id, payload|
  ...
  describe 'response truncation' do
    ...
    it 'truncates long response bodies' do
      described_class.deliver(delivery)
      expect(delivery.reload.response_body.length).to be <= 10_050 # 10000 + truncation message
