...
RSpec.describe Webhookable::Model do
  ...
  describe '#trigger_webhook' do
    ...
    it 'creates a webhook event' do
      ...
      end.to change { Webhookable::WebhookEvent.count }.by(1)
    ...
    it 'raises error for undefined events' do
      ...
      end.to raise_error(ArgumentError, /Event 'invalid_event' is not defined/)
    end
    ...
    it 'instruments the webhook trigger' do
      ...
      ActiveSupport::Notifications.subscribe('webhook.triggered') do |_name, _start, _finish, _id, payload|
  ...
  describe 'generated trigger methods' do
    ...
    it 'triggers webhook with the correct event' do
      ...
      end.to change { Webhookable::WebhookEvent.where(event_type: 'completed').count }.by(1)
    end
  ...
  describe '#webhook_events' do
    it 'returns webhook events for the record' do
      ...
      expect(events.pluck(:event_type)).to match_array(%w[completed cancelled])
    end
  end
